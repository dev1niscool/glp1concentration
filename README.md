...
The Variable injection dates tab also includes an **Interval calculator**. After plotting, enter a reference concentration in ng/mL; the numerical comparison ceiling is that value plus 20 ng/mL. The editable dose defaults to the chronologically latest plotted injection, regardless of block order. This is a model-only scenario search, not a personal tolerability assessment, a dosing recommendation, or a guarantee against side effects. Changes to dates or model inputs require replotting before another calculation.
