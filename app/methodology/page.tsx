    ...
    <main className="site-shell methodology-page">
      ...
      <section className="method-block" id="variable-dates">
        ...
        <div className="method-copy method-prose">
          ...
          <h3>Remembering inputs on this device</h3>
          <p>Each plotter tab automatically saves its input values in this browser’s local storage, including doses, dates, intervals, graph settings, body-size inputs, and interval-calculator entries. Complete saved inputs redraw the main graph when you return. Incomplete drafts can be completed without re-entering the other fields. Interval estimates and their continuation graphs are recalculated when requested; a saved numerical result is never reused.</p>
          <p>The calculator remembers an explicitly selected dose for its medication. Choosing “use latest entered dose” keeps the default linked to the most recent plotted injection. Reset clears the current tab’s saved inputs, including its calculator entries. These graph settings and calculator entries stay in this browser. Anyone using the same browser profile can see them; clearing site data also removes them. Browser settings or private browsing may prevent saving or retaining inputs, in which case the plotter remains usable and displays a storage notice.</p>
          <h3>My Profile, backups, and accounts</h3>
          <p><Link href="/my-profile">My Profile</Link> keeps an editable injection history with the same multiple-date dose blocks. Up to 100 dates are supported. Changes, including incomplete drafts, save automatically in this browser. Export downloads a JSON backup of the history; import accepts this site’s supported export format and lets you review it before replacing the browser copy. Complete valid dates are required for export and plotting.</p>
          <p>“Load from My Profile” in Variable injection dates copies the profile into the graph’s dose blocks. Plot the copied history to view its concentrations or use the interval calculator. Graph edits and modeled future injections do not change the profile. Importing a file or loading an account copy also does not automatically change an existing graph.</p>
          <p>Optional username/password accounts use Supabase to store injection histories across devices. Usernames are case-insensitive. No email is collected and there is no email verification or forgotten-password recovery, so keep your credentials securely and export backups. Choose “Save to account” after editing; on another device, sign in, load the account profile, and choose “Use account copy.” Only the injection history is uploaded by this action. Body-size inputs, graph settings, and calculator entries stay local.</p>
          <p>Account histories are restricted to their authenticated owner by database row-level security; the service and project administrators retain administrative access. An existing account copy is reviewed before replacement, and a save is rejected if another device has changed that copy since it was loaded. Export local edits before reloading after a conflict. Signing out retains the local browser history, so clear it on a shared device. Account service availability and free-plan limits do not prevent local editing or import/export.</p>
      ...
      <section className="method-block" id="limits">
        ...
        <div className="method-copy limit-copy">
          ...
          <ul>
            ...
            <li><strong>Custom intervals are descriptive only.</strong> The compounded and exact-date controls model the schedule entered; it does not establish that the interval is appropriate or safe.</li>
